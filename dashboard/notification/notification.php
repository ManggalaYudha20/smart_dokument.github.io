...
// query table Notifikasi
$queryAllNotification = query("SELECT * FROM tb_notification");

// ====================================================
// PENGKONDISIAN READ NOTIFICATION
// ====================================================
// READ NOTIFICATION SURAT MASUK
if (isset($_POST["read_notification"])) {
	if (readNotification($_POST) > 0) {
		echo "
    <script>
...
if (isset($_POST["read_all_notification"])) {
	if (readAllNotification($_POST) > 0) {
		echo "
    <script>
      document.location.href = '';
    </script>
...
// VIEW NOTIFIKASI FROM URL ID
if (isset($_GET["view"])) {
	$queryAllNotification = viewNotification(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["id_notif"]))))))));
}

// URL VALIDATION (SQL INJECTION)
if (isset($_GET["view"])) {
	if (empty($_GET["id_notif"])) {
		header("Location:../../");
		exit;
	}

	if (strlen($_GET["id_notif"]) < 10) {
		header("Location:../../");
		exit;
	}
	if (!base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($_GET["id_notif"])))))))) {
		header("Location:../../");
		exit;
	}
}
...
<head>
	...
	<title>Notifikasi - smart dokument</title>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-body">
									...
									<?php if (!empty($queryAllNotification)) : ?>
										<?php foreach ($queryAllNotification as $row) : ?>
											<div class="container-notification">

												<?php if ($row["notif"] === "tsm") : ?>
													<div class="icon-notification bg-success">
														<i class="fa fa-envelope"><sup class="fa fa-plus"></sup></i>
													</div>
												<?php endif; ?>
												<?php if ($row["notif"] === "tsk") : ?>
													<div class="icon-notification bg-danger">
														<i class="fa fa-envelope-open"><sup class="fa fa-plus"></sup></i>
													</div>
												<?php endif; ?>
												...
												<div class="description-notification">
													<p>
														...
														<?php endif; ?>
														<br>
														<strong>Nomor Surat : <?= $row["no_surat"]; ?></strong>
													</p>
													<span>
														<i class="fa fa-calendar"></i> <?= tgl_indo_notif(date($row["date"])); ?>
														<i class="fa fa-clock"></i> <?= substr($row["time"], 0, -3); ?>
													</span>
													<?php if ($row["notif"] === "tsm") : ?>
														<a href="../../<?php include("url.php"); ?>dashboard/transaksi-surat/surat-masuk">Lihat <i class="fa fa-eye"></i></a>
													<?php endif; ?>
													<?php if ($row["notif"] === "tsk") : ?>
														<a href="../../<?php include("url.php"); ?>dashboard/transaksi-surat/surat-keluar">Lihat <i class="fa fa-eye"></i></a>
													<?php endif; ?>
												</div>
											</div>
										<?php endforeach; ?>
